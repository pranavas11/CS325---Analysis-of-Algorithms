...
then
	...
	python3 stooge_sort.py > stooge.txt
	python3 stooge_sort.py
	echo
	echo "Merge Sort 3"

	python3 merge_sort3.py > merge3.txt
	python3 merge_sort3.py
  echo
	echo "Stooge Sort Comparison"
	diff -y -B -b --report-identical-files --suppress-common-lines stooge.txt sorted.txt
  echo
	...
	echo "Stooge Sort times"
	timeout 60s python3 stooge_time.py
	echo
	echo "Merge Sort 3 times"
	timeout 60s python3 merge3_time.py
else
    echo "No Python file"	
fi